...
export async function POST(request: NextRequest) {
  try {
    ...
    const { auditId, questionId, questionText, organizationId } = body;
    ...
    if (documents.length > 0) {
      documentContext = documents
        .flatMap((doc) =>
          doc.chunks.map((chunk) => `[From ${doc.name}]:\n${chunk.content}`)
        )
        .join('\n\n---\n\n');
    }

    // Add evidence descriptions if available
    if (response?.evidences?.length) {
      documentContext += '\n\n--- Evidence Files ---\n';
...
Analyze these documents and determine compliance with the question above.`;
    ...
    await prisma.aISuggestion.upsert({
      ...
      update: {
        ...
        sources: documents.map((d) => ({ documentName: d.name, documentId: d.id })),
      },
      create: {
        auditId,
        questionId,
